...
var ScriptingTools = class {
  ...
  getFileFromPath(path, filesCheck) {
    ...
    const matchingFiles = files.filter(
      (file) => file instanceof import_obsidian4.TFile && file.path.toLocaleLowerCase().includes(path.toLocaleLowerCase())
    );
...
// src/autocompleteModal.ts
var AutocompleteModal = class extends import_obsidian6.Modal {
  /**
  ...
  async onOpen() {
    var _a;
    this.knownProperties = await this.scriptingTools.fetchKnownProperties(this.app);
    const { contentEl } = this;
    ...
    for (const [key, value] of Object.entries(this.frontmatter)) {
      if (key.startsWith(this.rule.property + ".")) {
        ...
        const nameInput = new import_obsidian6.TextComponent(searchContainer).setValue(key).setPlaceholder("Property name").setDisabled(true);
...
async function openAutocompleteModal(app, plugin, rule, activeFile, frontmatter) {
...
var FileSuggest = class extends TextInputSuggest {
  ...
  getSuggestions(input_str) {
    ...
    all_files.forEach((file) => {
      if (file instanceof import_obsidian9.TFile && file.extension === "md" && file.path.toLowerCase().contains(lower_input_str)) {
        files.push(file);
      }
    });
...
var DirectorySelectionModal = class extends import_obsidian10.Modal {
  ...
  createModeSetting(containerEl) {
    if (this.includeExcludeSelectable) {
      this.modeDropdown = new import_obsidian10.Setting(containerEl).setName("Selection Mode").setDesc("Choose if the selected files and folders should be excluded or included.").addDropdown((dropdown) => {
  ...
  createShowFilesSetting(containerEl) {
    if (this.options.optionShowFiles) {
      new import_obsidian10.Setting(containerEl).setName("Show Files").setDesc("Show Files within the directory tree.").addToggle((toggle) => {
  ...
  createActionButtons(containerEl) {
    ...
    buttonsEl.style.gap = "10px";
    const resetButton = buttonsEl.createEl("button");
    (0, import_obsidian10.setIcon)(resetButton, "reset");
    resetButton.ariaLabel = "Reset selection";
    resetButton.onclick = () => {
      this.resetSelectionToInitial();
    ...
    const emptyButton = buttonsEl.createEl("button");
    (0, import_obsidian10.setIcon)(emptyButton, "eraser");
    emptyButton.ariaLabel = "Clear selection";
    emptyButton.onclick = () => {
...
async function getRuleResult(ruleFx, app, rule, ruleFunction, currentFile, tools2, frontMatter) {
  let result = void 0;
  switch (ruleFunction.ruleType) {
    case "script":
    case "buildIn":
      result = applyFormatOptions(ruleFx(app, currentFile, tools2), rule);
      break;
    case "buildIn.inputProperty":
      result = applyFormatOptions(ruleFx(app, currentFile, tools2, frontMatter[rule.inputProperty]), rule);
      break;
    case "autocomplete.modal":
      ruleFx(app, currentFile, tools2);
      console.log("autocomplete modal", ruleFx, ruleFunction, currentFile, tools2);
      result = null;
      break;
    case "automation":
      console.log("automation", ruleFunction, currentFile, tools2);
      result = applyFormatOptions(await ruleFx(app, currentFile, tools2), rule);
      break;
  }
  return result;
}
async function executeRule(event, app, settings, currentFile, returnResult, rule, frontMatter, oldPath) {
  if (!rule.active || !currentFile)
    return returnResult;
  const tools2 = new ScriptingTools(app, this, settings, rule, frontMatter, currentFile);
  let fxResult;
  let oldResult;
  let oldFile = void 0;
  if (oldPath) {
    ...
    switch (rule.content) {
      ...
      default:
        ...
        if (functionIndex !== -1) {
          ...
          fxResult = await getRuleResult(ruleFunction2, app, rule, ruleFunctions[functionIndex], currentFile, tools2, frontMatter);
          if (oldFile) {
            oldResult = await getRuleResult(ruleFunction2, app, rule, ruleFunctions[functionIndex], oldFile, tools2, frontMatter);
          }
        } else {
...
ruleFunctions.push({
  ...
  fx: function(app, file, tools2) {
    ...
    const addExtension = tools2.getOptionConfig((_a = tools2.getRule()) == null ? void 0 : _a.id, "addExtension");
    parts.pop();
  ...
  configTab: function(optionEL, rule, that, previewComponent) {
    ...
    new import_obsidian11.Setting(optionEL).setName("Include file extension").setDesc("Add file extension to pathname").addToggle(
...
ruleFunctions.push({
  ...
  fx: async function(app, file, tools2) {
    console.log(`autocomplete modal, work in progress...`);
    const currentContent = tools2.getCurrentContent();
    const rule = tools2.getRule();
    if (!rule)
      return currentContent;
    const options = tools2.getOptionConfig(rule.id);
    if (!rule)
      return tools2.getCurrentContent() || "autocomplete.modal";
    ...
    console.log("autocomplete modal result", result, tools2.getFrontmatter());
  ...
  configTab: function(optionEL, rule, that, previewComponent) {
    ...
    new import_obsidian11.Setting(optionEL).setName("Delimiter").setDesc("Character to determine which property should appear in the modal").addText((text) => text.setValue(that.getOptionConfig(rule.id, "propertyDelimiter") || ".").onChange(async (value) => {
...
ruleFunctions.push({
  ...
  fx: async function(app, file, tools2) {
    ...
    const options = tools2.getOptionConfig(rule.id);
    const filesToCheck = tools2.getFilesInVault(options.destinationFolder);
    ...
    for (const part of links) {
      let link = tools2.extractLinkParts(part);
      let linkFile = tools2.getFileFromPath(link.path, filesToCheck);
      if (!linkFile) {
        ...
        linkFile = await tools2.createFileFromPath(link.path, options.addTemplate ? options.templateFile : void 0);
        console.log(`autoLink: new file created ${linkFile.path}`);
        tools2.updateFrontmatter(rule.property, [`[[${tools2.removeLeadingSlash(link.path)}|${link.title}]]`], linkFile);
      } else {
        console.log(`autoLink: creating Link to existing File ${linkFile.path}`);
      }
      if (linkFile) {
        link.path = linkFile.path;
        newContent.push(`[[${tools2.removeLeadingSlash(link.path)}|${link.title}]]`);
      }
      console.log(`autoLink: new content`, newContent);
    }
    console.log(`autoLink: write content`, newContent);
    tools2.updateFrontmatter(rule.property, newContent);
    return newContent;
  },
  configTab: function(optionEL, rule, that, previewComponent) {
    that.setOptionConfigDefaults(rule.id, {
    ...
    new import_obsidian11.Setting(optionEL).setName("Add template to new files").setDesc("Automatically add template to new files").addToggle((toggle) => toggle.setValue(that.getOptionConfig(rule.id, "addTemplate") || false).onChange(async (value) => {
      that.setOptionConfig(rule.id, "addTemplate", value);
    }));
    new import_obsidian11.Setting(optionEL).setName("Ask for confirmation").setDesc("Ask for confirmation before creating new files").addToggle((toggle) => toggle.setValue(that.getOptionConfig(rule.id, "askConfirmation") || false).onChange(async (value) => {
      that.setOptionConfig(rule.id, "askConfirmation", value);
    }));
    let destinationFolderEl;
    new import_obsidian11.Setting(optionEL).setName("Destination Folder").setDesc("Folder to place new files").addSearch((cb) => {
    ...
    new import_obsidian11.Setting(optionEL).setName("Template File").setDesc("Select a template file to add to new files").addSearch((cb) => {
