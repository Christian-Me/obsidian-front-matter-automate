...
export class RuleReplaceSpecialChars extends RulePrototype {
    ...
    fx (app: App | undefined, file: any, tools: ScriptingTools, input?:any) { // Default function signature
        const specialCharReplacement = tools.getOptionConfig(tools.getRule()?.id, 'specialCharReplacement');
    ...
    configTab(optionEL: HTMLElement, rule: FrontmatterAutomateRuleSettings, that: any, previewComponent: any) {
        ...
        new Setting(optionEL)
            ...
            .addText(text => text
                ...
                .setValue(that.getOptionConfig(rule.id ,'specialCharReplacement') || '')
